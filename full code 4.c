...
void mainMenu();
void mineRecursive(struct mineboard(*m)[MAX], int tmpX, int tmpY, int *retMine); //수정
void rank();
void displayMap(struct mineboard(*m)[MAX], int gamelevel,int *retMine); //수정
int resultFunc(struct mineboard(*m)[MAX], int gamelevel, int allMine); 
void howtoPlay(); 
int speedBreaker(struct mineboard(*m)[MAX], int tmpX, int tmpY); //추가
...
void mineRecursive(struct mineboard(*m)[MAX], int tmpX, int tmpY, int* retMine) { //매개변수 추가
	int i, j;
	if (m[tmpX][tmpY].block == 1) return;
	else if (m[tmpX][tmpY].wall == 1) return;
	else if (m[tmpX][tmpY].minecount != 0) {
		m[tmpX][tmpY].block = 1;
		if (m[tmpX][tmpY].flag == 1) {        //추가
			m[tmpX][tmpY].flag = 0;
			++*retMine;
		}
		return;
		...
		for (i = tmpX - 1; i < tmpX + 2; i++) {
			for (j = tmpY - 1; j < tmpY + 2; j++) {
				mineRecursive(m, i, j, retMine);  //매개변수 수정
			}
		}
	}
}
...
void displayMap(struct mineboard(*m)[MAX], int gamelevel, int *retMine) { //매개변수 추가
	gotoxy(0, 0);
	for (int i = 0; i < gamelevel; i++) {
		for (int j = 0; j < gamelevel; j++) {
...
int resultFunc(struct mineboard(*m)[MAX], int gamelevel, int allMine) {
	...
	if (allMine == count) {
		displayMap(m, gamelevel,&allMine); //수정
...
int speedBreaker(struct mineboard(*m)[MAX], int tmpX, int tmpY) {  //추가된 기능
	int i, j;
	int result = 1;
		...
		if (_kbhit()) {
			...
			if (ch == 'f' || ch == 'F') {
				...
				if (m[player.y][player.x / 2].mine == 1) {
					...
					displayMap(m, gamelevel, &mineCnt);  //매개변수 수정
					gotoxy(gamelevel * 2 + 5, 6);
					printf("게임 실패");
					...
					mineRecursive(m, player.y, player.x / 2, &mineCnt);  //매개변수 수정
					displayMap(m, gamelevel, &mineCnt);
			...
			else if ((m[player.y][player.x / 2].block == 1) && (ch == 'd' || ch == 'D')) {  //기능 추가
					...
					for (int i = player.y - 1; i < player.y + 2; i++) {
						for (int j = player.x / 2 - 1; j < player.x / 2 + 2; j++) {
							if (m[i][j].mine == 0 && m[i][j].block == 0) {
								mineRecursive(m, i, j,&mineCnt);    //매개변수 수정
								displayMap(m, gamelevel,&mineCnt);
								if (resultFunc(m, gamelevel, mineCnt) == 1) {
									time(&end);
									duration = difftime(end, start);
